...
const (
	USER_NAME_DEFAULT    = "Имя"
	USER_SURNAME_DEFAULT = "Фамилия"
)

// Структура для хранения данных пользователя
type ViewUser struct {
	Login        string `json:"email" validate:"max=255,regexp=^.+@.+$"` // Логин пользователя
...
type ViewApiUserFull struct {
	...
	Language     string               `json:"language" db:"language" validate:"min=1,max=10"` // Язык пользователя по умолчанию
...
func NewApiUserShort(id int64, blocked bool, confirmed bool, lastlogin time.Time, surname string, name string,
	middlename string) *ApiUserShort {
	return &ApiUserShort{
		ID:         id,
		Blocked:    blocked,
		Confirmed:  confirmed,
